...
class Automate:
    ...
    def tweeting(self, tweet):
            ...
            tw = textwrap.TextWrapper(width=280)
            listtweet = tw.wrap(tweet)
            print(listtweet)

            if len(listtweet)==1:
                postspace.send_keys(listtweet[0])
            ...
            elif len(listtweet)>25:
                print ("You are too wordy. Twitter can take only 7000 words in a thread. Run this twice.")
            else:
                postspace.send_keys(listtweet[0])
                ...
                for n in range(1,len(listtweet)):
                    print (n)
                    postspace = self.driver.find_elements_by_class_name('public-DraftStyleDefault-block')
                    print(postspace)
                    postspace[n].send_keys(listtweet[n])
                    if n<len(listtweet)-1:
